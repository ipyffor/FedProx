...
with open(json_list[3], 'r') as f:
    metrics_dic = json.load(f)
    train_accuracies = metrics_dic['train_accuracies']
    for i, acc in enumerate(train_accuracies):
        # xticks.append(i)
        if show_all:
            acc_grad_random.extend([a/b for a,b in zip(acc[3], acc[2])])
        else:
            acc_grad_random.append(np.sum(acc[3]) * 1.0 / np.sum(acc[2]))
...
acc_grad_random = acc_grad_random[:sub_length:scale]
X = [ i for i in range(0,len(acc_grad))]

ln1, = plt.plot(X,acc_loss,color='red',linewidth=linewidth,linestyle='-')
ln2, = plt.plot(X,acc_grad,color='blue',linewidth=linewidth,linestyle='-')
ln3, = plt.plot(X,acc_random,color='green',linewidth=linewidth,linestyle='-')
ln4, = plt.plot(X,acc_grad_random,color='orange',linewidth=linewidth,linestyle='-')
xticks = xticks[::20]
...
plt.legend(handles=[ln1, ln2, ln3,ln4], labels=['根据loss选择用户', '根据grad选择用户', '随机选择用户', ' 根据梯度权重随机选择'], prop=my_font)
